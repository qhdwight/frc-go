...
struct Pigeon2_Faults {
	 ...
	 * This is typical when calibrating.
	 */
	...
	uint64_t ToBitfield() const {
		...
		deviceFaults |= MotionDriverTookTooLong ? 1 : 0; deviceFaults <<= 1;
		deviceFaults |= DataAcquiredLate ? 1 : 0; deviceFaults <<= 1;
	...
	void Update(uint64_t bits) {
		...
		DataAcquiredLate = (bits & mask) ? true : false; mask <<= 1;
		MotionDriverTookTooLong = (bits & mask) ? true : false; mask <<= 1;
